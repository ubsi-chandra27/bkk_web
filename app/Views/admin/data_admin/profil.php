...
$isDudi = (int) ($admin['id_role'] ?? 0) === 3;
$statusBadge = ((int) ($admin['is_active'] ?? 0) === 1) ? 'success' : 'danger';
$statusLabel = ((int) ($admin['is_active'] ?? 0) === 1) ? 'Aktif' : 'Nonaktif';
...
$jenisKelamin = static function ($value) {
    if ($value === 'L') {
        return 'Laki-laki';
    }
...
<div class="post d-flex flex-column-fluid" id="kt_post">
    <div id="kt_content_container" class="container-xxl">
        <div class="d-flex flex-wrap flex-stack mb-6">
            <div>
                <h1 class="fw-bolder text-dark mb-1">Profil Admin</h1>
                <div class="text-muted fw-semibold fs-6">Detail data admin dan relasi perusahaan DU/DI</div>
            </div>
            <div class="d-flex gap-2">
                <?php if (in_array(session('id_role'), [1, 2])): ?>
                    <a href="<?= base_url('admin/data-admin/' . ($admin['id'] ?? 0) . '/edit-profil') ?>" class="btn btn-sm btn-primary">
                        <i class="bi bi-pencil-square fs-6"></i> Edit Profil
                    </a>
                <?php endif; ?>
                <a href="<?= site_url('admin/data-admin') ?>" class="btn btn-sm btn-light-primary">
                    ...
                    <div class="flex-grow-1">
                            ...
                            <div class="d-flex flex-column">
                                ...
                                <div class="d-flex flex-wrap fw-bold fs-6 mb-4 pe-2">
                                    <span class="d-flex align-items-center text-gray-400 me-5 mb-2">
                                        <i class="bi bi-envelope me-2"></i><?= esc($formatValue($admin['email'] ?? null)) ?>
                                    </span>
                                    <?php if (!empty($admin['telepon'])): ?>
                                        <span class="d-flex align-items-center text-gray-400 me-5 mb-2">
                                            <i class="bi bi-telephone me-2"></i><?= esc($admin['telepon']) ?>
                                        </span>
                                    <?php endif ?>
                                    <span class="d-flex align-items-center text-gray-400 mb-2">
                                        <i class="bi bi-person-badge me-2"></i><?= esc($formatValue($admin['nama_role'] ?? null)) ?>
                                    </span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="d-flex flex-wrap">
                            <div class="border border-gray-300 border-dashed rounded min-w-125px py-3 px-4 me-6 mb-3">
                                ...
                                <div class="fw-bold fs-6 text-gray-400"><?= esc($formatValue($admin['nama_role'] ?? null)) ?></div>
                            </div>
                            <div class="border border-gray-300 border-dashed rounded min-w-125px py-3 px-4 me-6 mb-3">
                                <div class="fs-6 fw-bold text-gray-700">Bergabung</div>
                                <div class="fw-bold fs-6 text-gray-400"><?= esc($formatDate($admin['created_at'] ?? null)) ?></div>
                            </div>
                            <?php if (!empty($admin['last_login'])): ?>
                                <div class="border border-gray-300 border-dashed rounded min-w-125px py-3 px-4 me-6 mb-3">
                                    <div class="fs-6 fw-bold text-gray-700">Login Terakhir</div>
                                    <div class="fw-bold fs-6 text-gray-400"><?= esc($formatDate($admin['last_login'], true)) ?></div>
                                </div>
                            <?php endif ?>
                        </div>
                    </div>
            ...
            <div class="<?= $isDudi ? 'tab-pane fade show active' : '' ?>" id="tab_data_diri">
                <div class="card mb-5 mb-xl-10">
                    ...
                    <div class="card-body p-9">
                        <div class="row">
                            <?php $infoItem('Nama Lengkap', $formatValue($admin['nama'] ?? null), 'bi-person', 'primary'); ?>
                            <?php $infoItem('Email', $formatValue($admin['email'] ?? null), 'bi-envelope', 'info'); ?>
                            <?php $infoItem('No. HP', $formatValue($admin['telepon'] ?? null), 'bi-telephone', 'success'); ?>
                            <?php $infoItem('Username', $formatValue($admin['email'] ?? null), 'bi-at', 'warning'); ?>
                            <?php $infoItem('Role', $formatValue($admin['nama_role'] ?? null), 'bi-person-badge', 'danger'); ?>
                            <?php $infoItem('Jenis Kelamin', $jenisKelamin($admin['jenis_kelamin'] ?? null), 'bi-gender-ambiguous', 'primary'); ?>
                            <?php $infoItem('Tempat Lahir', $formatValue($admin['tempat_lahir'] ?? null), 'bi-house', 'success'); ?>
                            <?php $infoItem('Tanggal Lahir', $formatDate($admin['tanggal_lahir'] ?? null), 'bi-calendar', 'danger'); ?>
                            <?php $infoItem('Alamat', $formatValue($admin['alamat'] ?? null), 'bi-geo-alt', 'warning'); ?>
                        </div>
                    </div>
                </div>
            </div>
                            ...
                            <?php if (empty($perusahaan)): ?>
                                <div class="alert alert-warning d-flex align-items-center p-5 mb-0">
                                    <div class="d-flex flex-column">
                                        ...
                                        <span>Akun admin DU/DI ini belum terhubung dengan data perusahaan.</span>
                                    </div>
                                </div>
                            <?php else: ?>
