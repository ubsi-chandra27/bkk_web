    ...
    <div class="card mb-5 mb-xl-10">
        <div class="card-header border-0 pt-5">
            <h3 class="card-title align-items-start flex-column">
                <span class="card-label fw-bolder fs-3 mb-1">Edit Profil</span>
                <span class="text-muted mt-1 fw-semibold fs-7">Perbarui informasi akun Anda</span>
            </h3>
            <div class="card-toolbar">
                <a href="<?= base_url('admin/profil-saya') ?>" class="btn btn-sm btn-light">
        ...
        <div class="card-body py-5">
            <form action="<?= site_url('admin/profil-saya/update') ?>" method="POST" enctype="multipart/form-data" id="editProfilForm">
                <?= csrf_field() ?>
                <input type="hidden" name="old_foto" value="<?= esc($user['foto'] ?? '') ?>">
                <input type="hidden" name="id_perusahaan" value="<?= esc($user['id_perusahaan'] ?? '') ?>">
                        ...
                        <div class="border border-dashed rounded p-8 h-100 text-center">
                            <div class="mb-5" id="fotoPreviewContainer">
                                ...
                                if (!empty($fotoFile)) {
                                    ...
                                    } elseif (is_file(FCPATH . 'uploads/foto/' . $fotoFile)) {
                                        $fotoUrl = base_url('uploads/foto/' . $fotoFile);
                                    }
                                }
                                ?>
                                ...
                                <?php else: ?>
                                    <div class="symbol symbol-100px symbol-lg-140px symbol-fixed mx-auto" id="fotoPlaceholder">
                                        <span class="symbol-label bg-light-primary text-primary fw-bolder fs-1"><?= strtoupper(substr(trim($user['nama'] ?? 'A'), 0, 1)) ?></span>
                                    </div>
                                    <img src="" alt="Preview Foto" id="fotoPreview" class="img-fluid rounded d-none" style="max-height: 140px; object-fit: contain;">
                                <?php endif ?>
                            </div>
                            <div class="fw-bold fs-5 text-gray-800 mb-1"><?= esc($user['nama'] ?? 'Admin') ?></div>
                            <div class="text-muted fs-7 mb-6"><?= esc($user['nama_role'] ?? 'Admin') ?></div>
                        ...
                        <div class="row g-5">
                            ...
                            <div class="col-md-6 col-12">
                                <div class="d-flex flex-column mb-8 fv-row">
                                    ...
                                    <input type="text" class="form-control form-control-solid" name="telepon" value="<?= old('telepon', $user['telepon'] ?? '') ?>">
                                </div>
                            </div>
                            <div class="col-md-6 col-12">
                                <div class="d-flex flex-column mb-8 fv-row">
                                    <label class="d-flex align-items-center fs-6 fw-semibold mb-2">Username</label>
                                    <input type="text" class="form-control form-control-solid bg-light" value="<?= esc($user['email'] ?? '') ?>" readonly tabindex="-1">
                                </div>
                            </div>
                            <div class="col-md-6 col-12">
                                <div class="d-flex flex-column mb-8 fv-row">
                                    <label class="d-flex align-items-center fs-6 fw-semibold mb-2">Role</label>
                                    <input type="text" class="form-control form-control-solid bg-light" value="<?= esc($user['nama_role'] ?? '') ?>" readonly tabindex="-1">
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="row g-5">
                            <div class="col-md-6 col-12">
                                <div class="d-flex flex-column mb-8 fv-row">
                                    ...
                                    <input type="password" class="form-control form-control-solid" name="password" id="profilSayaPassword" pattern="(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[^A-Za-z0-9]).{8,}" title="Password harus minimal 8 karakter, mengandung huruf besar, huruf kecil, angka, dan simbol." autocomplete="new-password">
                                    <small class="text-muted fs-7">Kosongkan jika tidak ingin mengubah password</small>
                                </div>
                            </div>
                            <div class="col-12">
                                <div class="password-strength-meter" id="profilSayaPasswordMeter">
                                    <div class="strength-bar">
                                        <div class="strength-progress" id="profilSayaStrengthProgress" style="width: 0%; background-color: #e9ecef;"></div>
                                    </div>
                                    <div class="strength-text mt-2" id="profilSayaStrengthText">Kekuatan password: <span class="text-muted">Belum diisi</span></div>
                                    <ul class="password-requirements list-unstyled mt-2">
                                        <li id="profilSaya-req-length" class="text-muted fs-7"><i class="bi bi-x-circle me-1"></i> Minimal 8 karakter</li>
                                        <li id="profilSaya-req-uppercase" class="text-muted fs-7"><i class="bi bi-x-circle me-1"></i> Mengandung huruf besar (A-Z)</li>
                                        <li id="profilSaya-req-lowercase" class="text-muted fs-7"><i class="bi bi-x-circle me-1"></i> Mengandung huruf kecil (a-z)</li>
                                        <li id="profilSaya-req-number" class="text-muted fs-7"><i class="bi bi-x-circle me-1"></i> Mengandung angka (0-9)</li>
                                        <li id="profilSaya-req-symbol" class="text-muted fs-7"><i class="bi bi-x-circle me-1"></i> Mengandung karakter simbol (!@#$%^&* dll)</li>
                                    </ul>
                                </div>
                            </div>
                            <div class="col-md-6 col-12">
                        ...
                        <div class="row g-5">
                            ...
                            <div class="col-md-6 col-12">
                                <div class="d-flex flex-column mb-8 fv-row">
                                    ...
                                    <input type="text" class="form-control form-control-solid" name="no_telepon_perusahaan" value="<?= old('no_telepon_perusahaan', $perusahaan['no_telepon'] ?? '') ?>" required>
                                </div>
                            </div>
                            <div class="col-md-6 col-12">
                                <div class="d-flex flex-column mb-8 fv-row">
                ...
                <div class="d-flex justify-content-end gap-3">
                    <a href="<?= base_url('admin/profil-saya') ?>" class="btn btn-light">Batal</a>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        initPasswordMeter('profilSayaPassword', 'profilSayaStrengthProgress', 'profilSayaStrengthText', [
            'profilSaya-req-length', 'profilSaya-req-uppercase', 'profilSaya-req-lowercase', 'profilSaya-req-number', 'profilSaya-req-symbol'
        ]);
    });
</script>
<?= $this->endSection() ?>
